	...
	public boolean updateFight() {
		...
		if(this.isCurrentFighting) {
				...
				case 0:
					...
					menu.printLine("Vous portez un coup à l'ennemi avec une force de : "+finalAttackPlayer);
					...
					if(ennemyFighting.isAlive()) {
						...
						menu.printLine("L'ennemie est vaincu : VICTOIRE");
						isFightingOver = true;
					}
					break;
				case 1:
					int attackEnnemy = this.ennemyFighting.getAttack();
					menu.printLine("L'ennemi vous attaque avec une force de : "+attackEnnemy);
					this.playerFighting.reduceLife(attackEnnemy);
					if(this.playerFighting.isAlive()) {
						menu.printLine("Vous etes encore vivant avec : "+this.playerFighting.getLife()+" points de vie");
					}else {
